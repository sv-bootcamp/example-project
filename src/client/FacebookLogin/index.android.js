			...
			<View style = {styles.container}>
				<LoginButton
					onLoginFinished = {
						(error, result) => {
							if (error) {
								Alert.alert('알림',	'문제발생!', [{text: '확인', onPress: () => console.log('OK Pressed')}]);
							} else if (result.isCancelled) {
								Alert.alert('알림',	'로그인이 취소됨.', [{text: '확인', onPress: () => console.log('OK Pressed')}]);
							} else {
								this.getToken();
							}
						}
					...
					onLogoutFinished={
						() => {
							Alert.alert('알림', '로그아웃 되었습니다.', [{text: '확인', onPress: () => console.log('OK Pressed')}])
						}
					}
				/>
			</View>
